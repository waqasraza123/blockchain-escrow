...
- Release 6 has started in the control plane: `apps/api`, `packages/shared`, and `packages/db` now support party-scoped immutable milestone submissions, buyer-side immutable milestone review decisions (`APPROVED` / `REJECTED`), active-escrow gating against live Release 4 agreement truth, timeline-style workflow derivation from the latest submission plus latest review, and audit-backed submission/review records; release/refund/dispute transitions and worker-owned milestone state progression are still deferred.
...
- Release 6 business modules beyond seller submission plus buyer review/rejection capture, including release/refund paths, disputes, partner APIs, and reporting, are not implemented.
...
- Release 6 buyer review is now implemented for organization-side buyers against party-scoped seller submissions, but counterparty-authenticated seller submission intake is still absent from the API surface, so some deal orientations still rely on future external submission channels before the full milestone lifecycle is reachable end-to-end.
