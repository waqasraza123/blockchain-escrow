...
- Release 6 has started in the control plane: `apps/api`, `apps/worker`, `packages/shared`, and `packages/db` now support party-scoped immutable milestone submissions from both organization-authenticated sellers and counterparty-signature-authenticated sellers, buyer-side immutable milestone review decisions (`APPROVED` / `REJECTED`), immutable buyer-side settlement requests (`RELEASE` / `REFUND`) anchored to the latest reviewed seller submission, immutable submission review deadlines, worker-owned immutable deadline-expiry events for overdue latest submissions, active-escrow gating against live Release 4 agreement truth, milestone workflow summaries derived from the latest submission plus latest review/deadline state, grouped milestone timeline read models derived from immutable submission/review/settlement/deadline-expiry records, and audit-backed submission/review/settlement/deadline-expiry records; actual settlement execution and dispute transitions are still deferred.
