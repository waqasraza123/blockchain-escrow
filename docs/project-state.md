...
- Runtime apps: `apps/web` and `apps/admin` are Next.js shells, `apps/api` is a NestJS control-plane API, `apps/indexer` is the normalized chain-read service, and `apps/worker` now owns asynchronous Release 5 draft-activation reconciliation plus future side effects.
...
- Worker-side funding reconciliation and deal lifecycle writes beyond indexed live activation are not implemented.
- Worker side effects such as stale-pending escalations, notifications, and operator-facing reconciliation tooling are not implemented.
...
- Release 5 `ACTIVE` state persistence now depends on the worker reconciliation loop rather than API read paths; if the worker is down, API funding and escrow summaries still reflect indexed truth, but stored draft state can lag until reconciliation resumes.
