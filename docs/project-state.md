...
- Release 6 buyer review plus settlement-request capture are now implemented for organization-side buyers against party-scoped seller submissions, but actual release/refund execution still depends on future contract/indexer work, and counterparty-authenticated seller submission intake is still absent from the API surface, so some deal orientations still rely on future external submission channels before the full milestone lifecycle is reachable end-to-end.
