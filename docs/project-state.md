...
- Release 5 allowance checks, real spender/transfer validation, replacement handling, stale pending detection, worker-side funding reconciliation, and deal lifecycle writes beyond offchain acceptance, transaction tracking, indexed execution outcomes, and indexed live activation are not implemented.
...
- Release 5 now tracks submitted funding transactions, promotes drafts to `ACTIVE` from indexer-owned agreement linkage, and surfaces failed tracked funding calls from indexer-owned transaction outcomes, but allowance/readiness checks are still bounded by the current create-only contract surface.
