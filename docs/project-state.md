...
- Release 6 has started in the control plane: `apps/api`, `packages/shared`, and `packages/db` now support seller-side immutable milestone submissions with file attachments, active-escrow gating against live Release 4 agreement truth, and audit-backed submission records; buyer review, release/refund/dispute transitions, and worker-owned milestone state progression are still deferred.
...
- Release 6 milestone submission foundation in `apps/api`, `packages/shared`, and `packages/db` with immutable `DealMilestoneSubmission` records, attachment-file links, seller-side submission endpoints, milestone workflow listing, audit-backed `DEAL_MILESTONE_SUBMISSION_CREATED` records, and active-escrow gating that keys off live linked agreement truth instead of API read-time workflow mutation.
...
- Release 6 business modules beyond seller submission capture, including buyer review/approval/rejection, release/refund paths, disputes, partner APIs, and reporting, are not implemented.
...
- Release 6 milestone submission capture is currently authenticated only for the organization-side seller party. Counterparty-authenticated seller flows, buyer review state changes, and worker-owned milestone progression are still absent, so submission history is production-grade but not yet a full milestone lifecycle.
